import type { Save, GameHooks, Cosmetics, DailyExpedition, ChoiceTier, HudState, Mode, RunState, Biome, Skin, Unit, Enemy, Gate, Obstacle, Pickup, Arrow, EnemyShot, Summon, Particle, Ring, Floaty, Wave, BossMetrics, Boss, LevelEvent, Encounter, EncounterTrigger, RunStyle, GolemGrant, Chapter, Crystal, Modifiers, Mastery } from '../types/craftrush.js';
...
import { TUNE, BIOMES, SKINS, CAMERAS, TIERS, COSMETICS, winBonus, speedById, currentChapter, chapterUnlocked, chapterById } from './config.ts';
import { Camera, renderWorld, DrawQueue } from './engine.ts';
import { Audio } from './audio.ts';
import { CrowdMixin } from './crowd.ts';
import { LevelMixin } from './levelgen.ts';
import { CombatMixin, GOLEM_GRANT_PROGRESS } from './combat.ts';
import { BossMixin } from './boss.ts';
import { FxMixin } from './fx.ts';
import { RenderMixin } from './render.ts';
import {
...
type CrowdMethods = typeof CrowdMixin;
type LevelMethods = typeof LevelMixin;
type CombatMethods = typeof CombatMixin;
type BossMethods = typeof BossMixin;
type FxMethods = typeof FxMixin;
type RenderMethods = typeof RenderMixin;
export interface Game extends CrowdMethods, LevelMethods, CombatMethods, BossMethods, FxMethods, RenderMethods {
  canvas: HTMLCanvasElement; ctx: CanvasRenderingContext2D; save: Save; hooks: GameHooks;
  cam: Camera; queue: DrawQueue; state: RunState; paused: boolean; t: number; freeze: number;
  flashFx: number; level: number; mode: Mode; biome: Biome; menuScroll: number; destroyed: boolean;
  _timeouts: Set<ReturnType<typeof setTimeout>>; _inputCleanup: (() => void) | null;
  skin: Skin; cosmetic: { [K in keyof Cosmetics]: Cosmetics[K][number] | null };
  crowd: Unit[]; enemies: Enemy[]; gates: Gate[]; obstacles: Obstacle[]; pickups: Pickup[];
  arrows: Arrow[]; eshots: EnemyShot[]; summons: Summon[]; particles: Particle[]; rings: Ring[];
  floaties: Floaty[]; waves: Wave[]; bossMetrics: BossMetrics; boss: Boss | null; bossDead: boolean;
  bigs: Unit[][]; reserve: number; stars: number; _units: Unit[]; _reformDirty: boolean;
  playerX: number; playerZ: number; targetX: number; speed: number; redstone: number;
  runEmeralds: number; kills: number; bestCrowd: number; runRods: number; volleyT: number;
  chargeT: number; firing: boolean; charging: boolean; volleysFired: number; crowdPulse: number;
  power: Record<string, number>; events: LevelEvent[]; eventIdx: number; encounters: Encounter[];
  encounterTriggers: EncounterTrigger[]; runStyle: RunStyle; length: number;
  creditSigns: {z: number; text: string; sub: string}[]; golemHintShown: boolean;
  golemGrantIdx: number; golemGrantLog: GolemGrant[]; expedition: DailyExpedition | null;
  chapter: Chapter | null; crystals: Crystal[]; mut: Modifiers; mastery: Mastery | null;
  bossArrivalCrowd: number | null; runId: string; keys: Record<string, boolean>;
  _hudT?: number; _hud?: HudState;
}
export class Game {
  constructor(canvas: HTMLCanvasElement, save: Save, hooks: GameHooks) {
    this.canvas = canvas;
  ...
  refreshCosmetics() {
    const pick = <K extends keyof Cosmetics>(cat: K): Cosmetics[K][number] | null => {
  ...
  startRun(expedition: DailyExpedition | null = null, replayId: string | null = null) {
    this.resetRunState();
    this.runId = newRunId();
  ...
  _initInput() {
    const c = this.canvas;
    let dragging = false, lastX: number | null = null, downX: number | null = null, downY: number | null = null, downAt = 0, moved = false;
    // relative steer from a pointer delta (blocks per on-screen pixel)
    const steer = (px: number) => {
    ...
    const onPointerLeave = (e: PointerEvent) => { if (e.pointerType === 'mouse') lastX = null; };
    this.keys = {};
    const onKeyDown = (e: KeyboardEvent) => {
      this.keys[e.code] = true;
      if (e.code === 'Escape') { e.preventDefault(); this.hooks.onPause && this.hooks.onPause(); }
      const tag = (e.target as HTMLElement | null)?.tagName;
      const interactive = (e.target as HTMLElement | null)?.isContentEditable
  ...
  noteGate(good: boolean | null, risky = false, choiceTier: ChoiceTier | null = null) {
  ...
  hudState(): HudState {
    ...
    const h = this._hud || (this._hud = { boss: { name: '', hp: 0, max: 1, needRunners: null, phase: 1, phases: 1, shielded: false } } as HudState);
