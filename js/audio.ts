...
let currentTrack: MusicTrack | null = null, seqTimer: ReturnType<typeof setInterval> | null = null, seqStep = 0, nextNoteTime = 0;
let musicNodes: OscillatorNode[] = []; // scheduled music sources, stoppable on track switch
...
function tone({ f = 440, f2 = null, type = 'square', dur = 0.1, vol = 0.2, delay = 0, out = null }: {f?: number; f2?: number | null; type?: OscillatorType; dur?: number; vol?: number; delay?: number; out?: GainNode | null}) {
...
const SFX = {
  ...
  golem_ready(){ [523, 659, 784].forEach((f, i) => tone({ f, type: 'triangle', dur: 0.11, vol: 0.12, delay: i * 0.055 })); },
  golem_hit(){ tone({ f: 125, f2: 70, type: 'triangle', dur: 0.22, vol: 0.24 }); noise({ dur: 0.12, vol: 0.12, freq: 520 }); },
  golem_smash(){ tone({ f: 105, f2: 42, type: 'sawtooth', dur: 0.36, vol: 0.32 }); noise({ dur: 0.24, vol: 0.22, freq: 430 }); },
...
interface Track {bpm: number; bass: (number | null)[]; lead: (number | null)[]; hat: number[]; leadType: OscillatorType; bassType: OscillatorType; leadVol: number; bassVol: number}
...
function scheduleStep(tr: Track, step: number, time: number) {
...
export const Audio = {
  ...
  sfx(name: keyof typeof SFX, throttleMs = 0) {
