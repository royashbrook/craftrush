...
import type { Unit } from '../types/craftrush.js';
// Craft Rush crowd sim: worth-based, unbounded army with ladder tiers.
import { TUNE, TIERS } from './config.ts';
...
export const CrowdMixin = {
  ...
  setWorth(this: Game, total: number, fx = false): void {
  ...
  tierPop(this: Game, text: string, unit: Unit | undefined, color: string): void {
    if (!unit) return;
  ...
  addRunners(this: Game, n: number, silent = false): void {
    if (n <= 0) return;
  ...
  killRunners(this: Game, n: number, atX: number | null = null, atZ: number | null = null, mitigate = true): void {
  ...
  reform(this: Game): void { this._reformDirty = true; this.flushReform(); },

  // run the deferred placement at most once per frame
  flushReform(this: Game): void {
