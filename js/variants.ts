import type { Skin, Cosmetics, Tiers, Palette } from '../types/craftrush.js';
...
export function enumerateVariants(cfg: {SKINS?: Pick<Skin, 'id' | 'body' | 'palette'>[]; COSMETICS?: Partial<Pick<Cosmetics, 'cape'>>; TIERS?: Partial<Tiers>}, ids: string[]) {
  const { SKINS = [], COSMETICS = {}, TIERS = {} } = cfg;
  const known = new Set(ids);
  const out = new Map<string, {key: string; id: string; palette: Palette | null}>();

  
  const want = (id: string, palette: Palette | null = null) => {
