...
import { importSave, MAX_BACKUPS } from './config.ts';
import { parsePlayableSave, record, type PartialSave } from './pwa-safety.ts';
import type { Backup } from '../types/craftrush.d.ts';

type BackupState = { ok: boolean; raw: string | null; items: unknown[] };
...
function transferId(text: string) {
  let hash = 2166136261;
...
function decodeSaveCode(code: unknown): PartialSave | null {
  if (typeof code !== 'string' || !code.startsWith('CR1|')) return null;
  try {
    return parsePlayableSave(decodeURIComponent(escape(atob(code.slice(4))))).save;
  } catch {
    return null;
  }
}
...
function restoreBackupBytes(raw: string | null) {
  try {
...
function writeBackups(state: BackupState, items: unknown[]) {
...
function normalizedIncomingBackups(value: unknown): Backup[] {
  if (!Array.isArray(value)) return [];
  const out: Backup[] = [];
  for (const entry of value) {
    if (!record(entry)) continue;
    const decoded = decodeSaveCode(entry.code);
    if (!decoded || typeof entry.code !== 'string' || typeof entry.day !== 'string' || !entry.day || entry.day.length > 64) continue;
    out.push({
      day: entry.day,
      ts: typeof entry.ts === 'number' && Number.isFinite(entry.ts) ? entry.ts : 0,
      level: decoded.level,
      emeralds: typeof decoded.emeralds === 'number' && Number.isFinite(decoded.emeralds) ? decoded.emeralds : 0,
      code: entry.code,
    });
  }
  return out;
 ...
 * retention limit.
 */
function mergedBackups(existing: unknown[], code: string, decoded: PartialSave, incoming: Backup[]) {
  const usedDays = new Set(existing.map((entry) => field(entry, 'day')).filter(Boolean));
  const usedCodes = new Set(existing.map((entry) => field(entry, 'code')).filter(Boolean));
...
export function consumeMigration() {
  ...
  const code = record(payload) && typeof payload.s === 'string' ? payload.s : '';
  ...
  if (hasSave && backupState.ok
      && backupState.items.some((entry) => field(entry, 'code') === code)) {
    finishTransfer(id);
    return null;
  }
  ...
  if (hasSave) {
    ...
    const next = mergedBackups(
      ...
      code,
      decoded,
      normalizedIncomingBackups(field(payload, 'b')),
    );
