import type { MasteryGrade, ChapterMasteryRecord, PersistentMastery, Save, FinishedMastery, MasteryTarget, MasteryMerge, Chapter, Mastery, ChoiceTier, FinishFacts } from '../types/craftrush.js';
// Pure run-mastery bookkeeping. These facts explain play; they never change
// rewards, progression, or the save schema.

interface MasteryResultInput {chapter?: string | Pick<Chapter, 'id' | 'credits' | 'coda'> | null; expedition?: unknown; win?: boolean; bestCrowd?: number; mastery?: Partial<FinishedMastery> | null}
const GRADE_STEPS: [number, MasteryGrade, string][] = [
  [90, 'S', 'AMAZING!'],
  [78, 'A', 'AWESOME!'],
  [64, 'B', 'NICE RUN!'],
  [48, 'C', 'GOOD TRY!'],
...
const GRADE_ORDER: string[] = ['D', 'C', 'B', 'A', 'S'];
...
const BADGE_IDS = new Set<string>(BADGES.map((badge) => badge.id));
const record = (value: unknown): value is Record<string, unknown> => !!value && typeof value === 'object' && !Array.isArray(value);

function emptyChapterMastery(): ChapterMasteryRecord {
  return { bestGrade: null, bestCrowd: 0, badges: [] };
}

function normalizeChapterRecord(value: unknown): ChapterMasteryRecord {
  ...
  const bestGrade = typeof source.bestGrade === 'string' && source.bestGrade.trim()
    ? source.bestGrade
    : null;
  const bestCrowd = Number.isFinite(source.bestCrowd as number) && (source.bestCrowd as number) >= 0
    ? Math.floor(source.bestCrowd as number)
    : 0;
  const storedBadges = Array.isArray(source.badges)
    ? [...new Set(source.badges.filter((badge) => typeof badge === 'string' && badge) as string[])]
    : [];
 ...
 */
export function normalizeMasterySave(save: unknown): PersistentMastery {
  ...
  return mastery as unknown as PersistentMastery;
}
 ...
 * derivation forever.
 */
export function chapterMastery(save: Pick<Save, 'mastery'> | null | undefined, chapterId: string) {
...
function earnedBadgeIds(result: MasteryResultInput | null | undefined) {
  const run = (record(result?.mastery) ? result.mastery : {}) as Partial<FinishedMastery>;
  const earned = [];
...
function cloneChapterRecord(value: ChapterMasteryRecord) {
  return { ...value, badges: [...value.badges] };
}

export function masteryChapterEligible(chapter: MasteryResultInput['chapter']) {
  const id = typeof chapter === 'string' ? chapter : chapter?.id;
  return typeof id === 'string' && !!id
    && id !== 'credits'
    && id !== 'coda'
    && !(chapter as Pick<Chapter, 'credits'> | null | undefined)?.credits
    && !(chapter as Pick<Chapter, 'coda'> | null | undefined)?.coda;
}
 ...
 */
export function nextMasteryTarget(save: Pick<Save, 'mastery'>, chapterId: string): MasteryTarget {
 ...
 */
export function mergeChapterMastery(save: Save, result: MasteryResultInput): MasteryMerge {
...
export function createMastery(chapter: Chapter | null, startCrowd: number): Mastery {
  return {
    gateChoices: 0,
    goodGates: 0,
    badGates: 0,
...
export function recordGate(mastery: Mastery | null, good: boolean | null, risky = false, choiceTier: ChoiceTier | null = null) {
  if (!mastery) return 0;
  mastery.gateChoices++;
  if (good === true) {
    mastery.goodGates++;
...
export function recordDamage(mastery: Mastery | null, amount: number) {
  if (mastery) mastery.damageTaken += Math.max(0, amount || 0);
}

export function recordDodge(mastery: Mastery | null, near = false) {
  if (!mastery) return;
  mastery.dodges++;
  if (near) mastery.nearMisses++;
}

export function objectiveState(mastery: Mastery | null, facts: {finishCrowd?: number; finalCrowd?: number} = {}) {
...
export function finishMastery(mastery: Mastery | null, {
  win, finalCrowd, finishCrowd = finalCrowd, bestCrowd, kills,
}: FinishFacts): FinishedMastery {
