import type { Biome, RunStyle, SweepMotion, Mode, Modifiers, LevelEvent, Encounter, GateShape, AmbushEnemy } from '../types/craftrush.js';
...
const roundZ = (value: number) => Math.round(value * 10) / 10;
const clamp = (value: number, low: number, high: number) => Math.max(low, Math.min(high, value));

export function encounterRunStyle(biome: Partial<Biome>): RunStyle {
...
export function sweepObstacleX(baseX: number, motion: SweepMotion | null, playerZ: number) {
...
export function buildEncounterRun({
...
}: {level?: number; mode?: Mode; biome?: Partial<Biome>; mut?: Modifiers; seed?: number} = {}) {
  ...
  const gateAfter = (worth: number, gate: Pick<GateShape, 'op' | 'val'>) => gate.op === 'mul'
    ? worth * gate.val
    : gate.op === 'scale'
      ? Math.floor(worth * gate.val)
    : worth + gate.val;

  const decisionGate = (card: Encounter, event: GateShape & {type: 'gate'}, metadata: {parBefore: number; bestAfter: number; alternateAfter: number}) => addEvent(card, {
    ...event,
    meaningful: true,
  ...
  const addGatePair = (card: Encounter, kind: 'risk' | 'learn' | 'standard') => {
    ...
    const best: Pick<GateShape, 'op' | 'val'> = { op: 'mul', val: multiplier };
    ...
    const alternate: Pick<GateShape, 'op' | 'val'> = {
      op: 'scale',
    ...
    const bestEvent: GateShape & {type: 'gate'} = {
  ...
  const addRewardTrail = (card: Encounter, options: {lane?: number; count?: number; arc?: boolean} = {}) => {
  ...
  const addReliefGate = (card: Encounter) => {
  ...
  const addOpenChallenge = (card: Encounter, compact = false) => {
  ...
  const ambushBranch = (dangerZ: number, side: number, count: number) => Array.from({ length: count }, (_, i) => ({
    ...
    })).filter((enemy) => enemy.id) as AmbushEnemy[];

  const addForestFork = (card: Encounter, compact = false) => {
    const choiceZ = roundZ(card.startZ + 4);
  ...
  const addDesertSweep = (card: Encounter, compact = false) => {
  ...
  const addEnemyRoute = (card: Encounter, compact = false) => {
  ...
  const addIdentityChallenge = (card: Encounter, compact = false) => {
