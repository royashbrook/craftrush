...
export type PartialSave = Partial<Save> & { level: number };
...
export function ownsCraftRushCache(name: unknown) {
 ...
 */
export function replayableCachedResponse(response: Response) {
 ...
 */
export function craftRushScopePath(pageHref: string) {
  try {
...
export function ownsCraftRushRegistration(registration: Pick<ServiceWorkerRegistration, 'scope'>, pageHref: string) {
  try {
    const page = new URL(pageHref);
...
export function isStandaloneApp(
  navigatorLike: { standalone?: boolean } = globalThis.navigator as Navigator & { standalone?: boolean },
 ...
 * an established save.
 */
export function shouldOfferLegacyRestore(save: unknown, standalone: boolean) {
  if (!standalone || !record(save)) return false;
  const done = record(save.campaign) && Array.isArray(save.campaign.done) ? save.campaign.done : [];
  const unlocked = Array.isArray(save.unlocked) ? save.unlocked : [];
  const runs = record(save.stats) && typeof save.stats.runs === 'number' && Number.isFinite(save.stats.runs) ? save.stats.runs : 0;
  const earned = record(save.stats) && typeof save.stats.totalEmeralds === 'number' && Number.isFinite(save.stats.totalEmeralds) ? save.stats.totalEmeralds : 0;
  return save.level === 1
    && (typeof save.bestLevel === 'number' ? save.bestLevel : 1) <= 1
    && done.length === 0
    && runs <= 1
    && earned <= 50
...
const stringArray = (value: unknown): value is string[] => Array.isArray(value) && value.every((item: unknown) => typeof item === 'string');
const finiteAtLeast = (value: unknown, min: number) => typeof value === 'number' && Number.isFinite(value) && value >= min;
function friendSchemaError(friend: unknown, path: string): string {
  ...
  for (const category of ['cape', 'hat']) {
    if (record(friend.cosmetics) && category in friend.cosmetics
...
export function saveSchemaError(save: unknown): string {
  ...
  if ('mode' in save && save.mode !== 'shooter' && save.mode !== 'gates') return 'mode must be shooter or gates';
  ...
  if (record(save.campaign) && 'done' in save.campaign && !stringArray(save.campaign.done)) {
    return 'campaign.done must be a list of chapter names';
  }
  if (record(save.mastery)) {
  ...
  if (record(save.world)) {
    ...
    if ('house' in save.world && (typeof save.world.house !== 'number' || !Number.isInteger(save.world.house) || save.world.house < 0)) {
    ...
    for (const [town, value] of Object.entries(save.world.towns || {})) {
      ...
      for (const house of Array.isArray(value.houses) ? value.houses : []) {
...
export function parsePlayableSave(json: string): { save: PartialSave; error: '' } | { save: null; error: string } {
  let save: unknown;
  try {
  ...
  // This is the sole assertion at the JSON boundary, after the complete schema check.
  return error ? { save: null, error } : { save: save as PartialSave, error: '' };
}

export function updateReloadIsSafe(nav: { playing: boolean; result: unknown }) {
