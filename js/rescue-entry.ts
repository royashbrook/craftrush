...
import { encodeSave, decodeSave, saveLink, codeFromHash } from './savecode.ts';
import {
  ownsCraftRushCache,
...
type RescueElements = { box: HTMLTextAreaElement; restoreBox: HTMLTextAreaElement; qrLink: HTMLInputElement; qr: HTMLCanvasElement };
function $<K extends string>(id: K): K extends keyof RescueElements ? RescueElements[K] : HTMLElement {
  const element = document.getElementById(id);
  if (!element) throw new Error(`Missing rescue element: ${id}`);
  // The self-contained template owns these four element types.
  return element as K extends keyof RescueElements ? RescueElements[K] : HTMLElement;
}
const errorMessage = (error: unknown) => error instanceof Error ? error.message : String(error);

// A failed read is not an empty slot. In particular, restoration must not
// replace bytes that it could not read and preserve first.
const readSave = () => localStorage.getItem(SAVE_KEY);
function message(id: string, kind: string, text: string) {
...
function decodeBackup(entry: unknown): string | null {
  try {
    if (record(entry) && typeof entry.code === 'string') return decodeBackup(entry.code);
...
function backupRows(list: unknown): { label: string; raw: string | null }[] {
  if (Array.isArray(list)) {
    return list.map((entry, index) => ({
      label: record(entry) && typeof entry.day === 'string' && entry.day ? entry.day : `backup ${index + 1}`,
...
function showBackups() {
  ...
  const rows: { label: string; raw: string | null }[] = [];
  if (previousRaw) {
    try {
  ...
  for (const row of rows) {
    ...
    let emeralds = '';
    try {
      const parsed = JSON.parse(row.raw ?? 'null');
 ...
 */
function preserveCurrentSave(raw: string) {
  localStorage.setItem(PRE_RESTORE_KEY, JSON.stringify({ ts: Date.now(), raw }));
  const written = JSON.parse(localStorage.getItem(PRE_RESTORE_KEY) ?? 'null');
...
function show() {
  describeContext();
  let raw;
  try { raw = readSave(); } catch {
    message('status', 'bad', 'Could not read saved progress in this browser.');
    $('summary').textContent = 'Nothing has been replaced. Allow browser storage and try again.';
    showBackups();
    return;
  }
  if (raw) {
    ...
    if (playable.error) {
      ...
      message('status', 'ok', 'FOUND YOUR SAVE');
...
async function showQR() {
  try {
    const raw = readSave();
    if (!raw) { message('qrMsg', 'bad', 'No save here to share.'); return; }
    ...
    reason.textContent = errorMessage(e);
...
async function takeIncoming() {
  try {
    ...
    message('msg2', 'bad', `That scanned code did not work: ${errorMessage(e)}`);
  }
}
...
function wire() {
  $('copy').addEventListener('click', async () => {
    ...
    let done = false;
    message('msg1', '', 'Copying…');
    try { if (navigator.clipboard) { await navigator.clipboard.writeText(t.value); done = true; } } catch { /* fall through */ }
    if (!done) { try { done = document.execCommand('copy'); } catch { /* ignore */ } }
    message('msg1', done ? 'ok' : 'bad', done
  ...
  $('restore').addEventListener('click', () => {
    ...
    try {
      ...
      // confirm() yields control. Preserve the latest bytes, not the stale
      // pre-dialog snapshot. A read error at either point stops replacement.
      current = readSave();
    } catch (e) {
      message('msg2', 'bad', `Nothing was replaced because the current save could not be read: ${errorMessage(e)}`);
      return;
    }
    if (current) {
      try {
        ...
        message('msg2', 'bad', `Nothing was replaced because the current save could not be backed up: ${errorMessage(e)}`);
        return;
      }
    }
    try {
      ...
      message('msg2', 'bad', `Could not write the restored save: ${errorMessage(e)}`);
      return;
    }
  ...
  $('clean').addEventListener('click', async () => {
    ...
    try {
      ...
      message('msg3', 'bad', `Something went wrong: ${errorMessage(e)}`);
    }
  });
}
