...
import type { LevelEvent } from '../types/craftrush.js';
// Craft Rush procedural level generation: event timeline + spawning.
import { TUNE, ENEMY_TYPES } from './config.ts';
import { buildEncounterRun, sweepObstacleX } from './encounters.ts';

export const LevelMixin = {
  genLevel(this: Game, diff: number): void {
  ...
  updateEncounterRuntime(this: Game): void {
    if (this.state !== 'run') {
  ...
  spawnEnemy(this: Game, id: string, x: number, z: number, options: {routeSide?: number} = {}): void {
