...
import type { Enemy, Mastery, Summon, Gate, Arrow, Obstacle, Pickup, Position } from '../types/craftrush.js';
// Craft Rush combat: enemies, arrows, gates, obstacles, pickups, summons, waves.
import { TUNE, ENEMY_TYPES, TIERS, PICKUPS } from './config.ts';
...
export const CombatMixin = {
  // ---------- combat ----------
  damageEnemy(this: Game, e: Enemy, dmg: number, silent = false): void {
    if (e.dead) return;
    e.hp -= dmg;
    e.flash = 0.09;
  ...
  explode(this: Game, x: number, z: number, radius: number, kills: number, hurtEnemies = true): void {
    Audio.sfx('boom');
  ...
  fireVolley(this: Game): void {
    ...
    const targets: Position[] = this.enemies.filter(e => !e.dead
  ...
  ensureGolemMastery(this: Game): Mastery {
    if (!this.mastery) this.mastery = {} as Mastery;
    for (const key of ['golemSends', 'usefulGolems', 'golemHits'] as const) {
      if (!Number.isFinite(this.mastery[key]) || this.mastery[key] < 0) this.mastery[key] = 0;
    }
    return this.mastery;
  },

  gainGolemCharge(this: Game, progress = 0): boolean {
  ...
  updateGolemChargeProgress(this: Game, progress: number): void {
  ...
  summonGolem(this: Game, source = 'manual'): void {
  ...
  recordGolemImpact(this: Game, s: Summon, x: number, z: number): boolean {
  ...
  // ---------- gates ----------
  gateLabel(this: Game, gt: Pick<Gate, 'op' | 'val'>): string {
    return gt.op === 'add' ? `+${gt.val}` : ['mul', 'scale'].includes(gt.op) ? `×${gt.val}` : gt.op === 'sub' ? `−${gt.val}` : `÷${gt.val}`;
  },

  gateGood(this: Game, gt: Pick<Gate, 'op'>): boolean { return gt.op === 'add' || gt.op === 'mul' || gt.op === 'scale'; },

  applyGate(this: Game, gt: Gate): void {
    gt.used = true;
  ...
  shootGate(this: Game, gt: Gate): void {
  ...
  updateArrows(this: Game, dt: number): void {
  ...
  arrowHitTest(this: Game, a: Arrow): void {
    for (const e of this.enemies) {
  ...
  breakObstacle(this: Game, o: Obstacle): void {
    this.burst(o.x, 0.6, o.z, ['#8a6844', '#6b4f35', '#a58a5a'], 10);
    Audio.sfx('pop', 40);
  },

  openChest(this: Game, p: Pickup): void {
    p.dead = true;
  ...
  updateEnemies(this: Game, dt: number): void {
  ...
  updateGatesObstaclesPickups(this: Game, dt: number): void {
    const px = this.playerX, pz = this.playerZ;
  ...
  collect(this: Game, p: Pickup): void {
    const def = PICKUPS[p.kind];
    if (!def) return;
  ...
  updateSummons(this: Game, dt: number): void {
  ...
  updateWaves(this: Game, dt: number): void {
  ...
  updateEshots(this: Game, dt: number): void {
    const px = this.playerX, pz = this.playerZ;
