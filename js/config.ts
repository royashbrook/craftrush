import type { Theme, CameraPreset, Pickup, Save, Cosmetic, Backup } from '../types/craftrush.js';
...
import { normalizeMasterySave } from './mastery.ts';
import { record, saveSchemaError } from './pwa-safety.ts';
...
const T = <K extends keyof Theme>(name: K, fallback: Partial<Theme[K]>): Theme[K] => (THEME[name] === undefined ? fallback : THEME[name]) as Theme[K];
export { THEME_INFO, THEME_ART, THEME_ATLAS, THEME_ID, THEME_ERROR } from './theme.ts';
...
export const speedById = (id: string) => SPEEDS.find((s) => s.id === id) || SPEEDS[1];
...
interface PickupDefinition {sprite: string; worldH: number; magnet?: boolean; grounded?: boolean; shooterAuto?: boolean; onCollect: (game: Game, pickup: Pickup) => void}
...
const POWERUP_NAMES: Record<string, string> = { triple: 'TRIPLE SHOT!', rapid: 'RAPID FIRE!', power: 'POWER SHOT!', sword: 'SWORD TIME!', axe: 'AXE TIME!' };
...
export function questCosmeticEarned(save: Save | null, def: Cosmetic | null | undefined) {
  if (!def || !def.quest) return true;
  return ((save && save.inventory && save.inventory[def.quest]) || 0) > 0;
}

export const chapterById = (id: string) => CAMPAIGN.find((c) => c.id === id);
export const chapterIndex = (id: string) => CAMPAIGN.findIndex((c) => c.id === id);

// what the save owes a chapter before it will open
export function chapterMissing(save: Save | null, id: string) {
...
export function chapterUnlocked(save: Save | null, id: string) {
...
export function currentChapter(save: Save | null) {
  const done = (save && save.campaign && save.campaign.done) || [];
...
export function completeChapter(save: Save, id: string) {
  const c = chapterById(id);
  if (!c) return null;
...
export function expeditionStatus(save: Save, key = dayKey()) {
...
export function recordExpedition(save: Save, key = dayKey()) {
...
export type SaveStatus = 'saved' | 'corrupt' | 'blocked' | 'conflict' | 'replaced' | 'backup-failed';
let saveStatus: SaveStatus = 'saved';
let expectedRaw: string | null | undefined;
let refused = false;
const saveListeners = new Set<(status: SaveStatus) => void>();
export const getSaveStatus = () => saveStatus;
function reportSave(status: SaveStatus) {
  saveStatus = status;
  for (const listener of saveListeners) listener(status);
}
...
function normalizeUnlockedSkins(save: Save) {
...
function defaultSave(): Save {
  const def: Save = { emeralds: 0, level: 1, bestLevel: 1, mode: 'shooter', skin: 'steve',
...
function normalizeSave(value: Record<string, unknown>): Save {
  const def = defaultSave();
  // Active additive records gain new defaults. Retired home/mine/world payloads
  // remain opaque, so an update does not rewrite a system no longer in use.
  const save = { ...def, ...value } as Save;
  for (const key of ['stats', 'cosmetics', 'inventory', 'campaign', 'expedition'] as const) {
    if (record(value[key])) Object.assign(save, { [key]: { ...def[key], ...value[key] } });
  }
...
export function loadSave(): Save {
  ...
  refused = false;
  try {
    expectedRaw = localStorage.getItem(SAVE_KEY);
    if (expectedRaw !== null) {
      let value: unknown;
      try { value = JSON.parse(expectedRaw); } catch { value = null; }
      if (!record(value)) { refused = true; reportSave('corrupt'); return save; }
      const candidate = normalizeSave(value);
      // Keep the existing in-memory mastery repair, but never silently replace
      // the original malformed bytes with its repaired interpretation.
      refused = !!saveSchemaError(value);
      if (saveSchemaError(candidate)) { refused = true; reportSave('corrupt'); return save; }
      save = candidate;
    }
    reportSave(refused ? 'corrupt' : 'saved');
  } catch { expectedRaw = undefined; refused = true; reportSave('blocked'); }
  return save;
}
...
export function winBonus(level: number, bestPower: number) {
...
export function persistSave(save: Save): boolean {
  if (expectedRaw === undefined && !refused) loadSave();
  if (refused) return false;
  try {
    // A stale tab must not overwrite an import/reset or progress from another
    // page. This synchronous read-before-write is not a cross-process CAS.
    if (localStorage.getItem(SAVE_KEY) !== expectedRaw) {
      refused = true; reportSave('conflict'); return false;
    }
    if (saveSchemaError(save)) { reportSave('blocked'); return false; }
    const raw = JSON.stringify(save);
    localStorage.setItem(SAVE_KEY, raw);
    if (localStorage.getItem(SAVE_KEY) !== raw) { refused = true; reportSave('conflict'); return false; }
    expectedRaw = raw;
    reportSave('saved');
    return true;
  } catch { reportSave('blocked'); return false; }
}

// ---- backup codes (Cookie-Clicker style) + reset ----
// A save code is base64(JSON) with a short "CR1|" prefix so it's recognizable.
export function exportSave(save: Save) {
  try { return 'CR1|' + btoa(unescape(encodeURIComponent(JSON.stringify(save)))); } catch { return ''; }
}

export function importSave(code: string): Save | null {
  try {
    const raw = String(code).trim().replace(/^CR1\|/, '');
    const obj = JSON.parse(decodeURIComponent(escape(atob(raw))));
    if (saveSchemaError(obj)) return null;
    const merged = normalizeSave(obj); // imports fill defaults, never borrow unrelated current progress
    ...
    expectedRaw = incoming;
    // The current page still holds its old reactive save until reload. Retire
    // its write authority now, including daily backups, rather than allowing a
    // quick settings change to overwrite the successfully imported progress.
    refused = true;
    reportSave('replaced');
    return merged;
  } catch { return null; }
...
function readBackups(): { raw: string | null; items: Backup[] } {
  const raw = localStorage.getItem(BACKUP_KEY);
  const value: unknown = raw === null ? [] : JSON.parse(raw);
  if (!Array.isArray(value) || !value.every((item: unknown) => record(item)
    && typeof item.day === 'string' && typeof item.code === 'string'
    && typeof item.ts === 'number' && Number.isFinite(item.ts)
    && typeof item.level === 'number' && Number.isFinite(item.level)
    && typeof item.emeralds === 'number' && Number.isFinite(item.emeralds))) {
    throw new Error('Unreadable backups');
  }
  return { raw, items: value as Backup[] };
}

export function listBackups(): Backup[] {
  try { return readBackups().items.slice().sort((a, b) => (a.day < b.day ? 1 : -1)); }
  catch { reportSave('backup-failed'); return []; }
}

// snapshot today's save; replaces today's existing entry, prunes the oldest
export function writeBackup(save: Save, now = Date.now()) {
  if (expectedRaw === undefined && !refused) loadSave();
  if (refused) return [];
  try {
    // A refused page must not erase today's recovery copy through a second
    // storage key after the main-slot ownership check kept it out.
    if (localStorage.getItem(SAVE_KEY) !== expectedRaw) {
      refused = true; reportSave('conflict'); return [];
    }
    const { raw, items } = readBackups();
    const day = dayStamp(now);
    const entry = { day, ts: now, level: save.level, emeralds: save.emeralds, code: exportSave(save) };
    const kept = items.filter(b => b.day !== day).sort((a, b) => (a.day < b.day ? 1 : -1));
    const out = [entry, ...kept].slice(0, MAX_BACKUPS);
    if (!entry.code || localStorage.getItem(BACKUP_KEY) !== raw) throw new Error('Backups changed');
    const next = JSON.stringify(out);
    localStorage.setItem(BACKUP_KEY, next);
    if (localStorage.getItem(BACKUP_KEY) !== next) throw new Error('Backup not confirmed');
    return out;
  } catch { reportSave('backup-failed'); return []; }
}

export function restoreBackup(day: string) {
...
export function resetSave() {
  try {
    ...
    if (localStorage.getItem(SAVE_KEY) !== null) throw new Error('Save removal not confirmed');
    expectedRaw = null;
    refused = true;
    reportSave('replaced');
    return true;
