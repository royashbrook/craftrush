...
import { loadSave, persistSave } from '../../js/config.ts';
import type { Save, HudState, RunResult } from '../../types/craftrush.js';
import type { ACHIEVEMENTS } from '../../js/achievements.ts';

export type Screen = 'menu' | 'pause' | 'shop' | 'more' | 'about' | 'goals' | 'settings' | 'help' | 'install';
export type ShopCategory = 'skin' | 'cape' | 'hat' | 'trail' | 'pet';
export type { UpdateStatus as UpdateState } from './update.ts';
...
interface Navigation {
  ...
  achPop: Achievement | null;
  hud: HudState | null;
  shopCategory: ShopCategory;
  restoreIntent: boolean;
}
interface ScreenDefinition {
  tab?: 'play' | 'shop' | 'settings';
  title: string;
  parent?: Screen;
}

export const save = $state<Save>(loadSave());
 ...
 */
export const nav = $state<Navigation>({
  screen: 'menu',
  stack: [],
...
export const SCREENS: Record<Screen, ScreenDefinition> = {
  ...
  help:      { tab: 'settings', title: 'How to Play', parent: 'more' },
  install:   { tab: 'settings', title: 'Add to Home Screen', parent: 'more' },
};
 ...
 */
export function go(screen: Screen, { push }: { push?: boolean } = {}) {
 ...
 */
export function initHistory(pushState: (url: string, state: { cr: boolean }) => void, pauseRun = () => togglePause(true)) {
